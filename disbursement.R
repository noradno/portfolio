...
# Include frame agreements in agreement_info
df_agreement_totals <-
  read_csv2(
...
df_agreement_info <- bind_rows(df_agreement_info, df_frameagreement_totals)
...
# Add frame agreements sums as separate rows, just like in statys_active
# And as in statsys_aktive, not includeing a parent_agreement_number, but a column to identify parent agreements (and possibly sub-frame agreements).

# We should start by creating a column to identify the subframe agreements
...
readr::write_csv2(df_agreement_info, "output/agreement_info.csv")
readr::write_csv2(df_agreement_disbursement, "output/agreement_disbursement.csv")
